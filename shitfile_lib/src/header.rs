use super::{checksum::checksum, error::ChunkError, file::SHITFILE};
...
impl Header {
    ...
    const MAGIC_BYTES: [u8; 10] = [83, 72, 73, 84, 70, 73, 76, 69, 00, 00];
...
impl TryFrom<Vec<u8>> for Header {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        if header != SHITFILE::MAGIC_BYTES {
...
mod header_tests {
    ...
    fn test_from_bytes_invalid_header() {
        let bytes = vec![
            69, 72, 73, 84, 70, 73, 76, 69, 0, 0, 0, 0, 0, 0, 0, 0, 0, 20, 0, 0, 0, 0, 0, 0, 0, 20,
            0, 0, 11, 80,
        ];
    ...
    fn test_from_bytes_checksum_fail() {
        let bytes = vec![
            83, 72, 73, 84, 70, 73, 76, 69, 0, 0, 0, 0, 0, 0, 0, 0, 0, 255, 0, 0, 0, 0, 0, 0, 0,
            20, 0, 0, 11, 80,
        ];
